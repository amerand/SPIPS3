...
def ReadAllFiles_Imu(directory, TeffMax=12000):
    ...
    files = [x for x in files if x.endswith('odfnew.dat')]
    if len(files)==0:
...
def ReadAllFiles_BOSZ():
    ...
    files = [x for x in files if x.endswith('.asc')]
    res = {}
    for f in files:
...
def getAllFiles(directory='gridp00k2odfnew', table=None, TeffMax=12000, TeffMin=4000):
    ...
    print(url+table)
    lines = urllib.request.urlopen(url+table).read().split('\n')
    ...
    for l in lines:
        ...
        if teff<=TeffMax and teff>=TeffMin:
            print(directory+'/'+l)
            tmp = urllib.request.urlopen(url+'/'+l, timeout=60).read()
...
try:
    tmp = len(list(models.keys()))
    print(tmp, 'ATLAS9 models already loaded')
except:
    # -- reading ATLAS9 models
    try:
        # -- read binary file
        print('ATLAS9 reading file', gridfile, end=' ')
