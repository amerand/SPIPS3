    ...
    for k,f in enumerate(files):
        ...
        if Teff>=TeffMin and Teff<=TeffMax:
            if Teff%200==0 and \
                ...
                print('downloading %3d/%3d'%(k+1, len(files)), end=' ')
                print(f)
                ftp.retrbinary('RETR '+f,
...
def UDLD(o, diam, Teff):
    ...
    if key0=='': # -- could not find the instrument, use wavelength
        ...
        # -- closest band
        wl0 = list(bands.keys())[np.argmin(np.abs(np.array(list(bands.keys()))-wl))]
        key0 = bands[wl0]
        wl1 = list(bands.keys())[np.argsort(np.abs(np.array(list(bands.keys()))-wl))[1]]
        key1 = bands[wl1]

    # -- interpolates in Teff:
    t0 = list(_data.keys())[np.abs(np.array(list(_data.keys()))-Teff).argmin()]
    t1 = list(_data.keys())[np.abs(np.array(list(_data.keys()))-Teff).argsort()[1]]
...
band = {wl_b[c]:c for c in list(wl_b.keys())}
...
def _udld(filename, plot=False, planar=False, showNeilson=False, reslim=0.05):
    ...
    if not planar:
        ...
        ross = np.mean([ross[k] for k in list(ross.keys())])
...
def compareModels(model=None, band='H', xmin=0.95):
    ...
    d = [(k[0]-_mdata['Teff'])**2/1e6 +
         (k[1]-_mdata['logg'])**2 +
         (k[2])**2 + (k[3]-2.)**2 for k in list(C4[c4_band].keys())]
    k = list(C4[c4_band].keys())[np.argmin(d)]
...
def fitDiamModel(oifits, model=None, figure=0, bootstrapping=False, firstGuess=3.0,
    ...
    if bootstrapping:
        ...
        for i,s in enumerate(itertools.combinations_with_replacement(stations, len(stations))):
            datap = {k:[] for k in list(oidata.keys())}
            for ij in s:
                for k in list(datap.keys()):
...
def v2Lin(bw, param):
    if 'R' in list(param.keys()):
        res = 0.
        d = {k:param[k] for k in list(param.keys()) if k!='R'}
...
reloadData = False
try:
    reloadData = len(list(_data.keys()))==0
except:
    reloadData = True
if reloadData:
    _data={}    
    ...
    if os.path.exists(os.path.join(_data_dir, datafile)):
        f = open(os.path.join(_data_dir, datafile), 'rb')
        _data = pickle.load(f)
        f.close()
    else:
