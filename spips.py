...
def fit(allobs, first_guess, doNotFit=None, guessDoNotFit=False, fitOnly=None, follow=None,
    ...
    obs = [o for o in allobs if o[-1]>0]
    ...
    if guessDoNotFit:
        ...
        doNotFit = ['MJD0','METAL']
        doNotFit.extend([x for x in list(first_guess.keys()) if 'COMP 'in x])
        ...
        if not any(['mag' in o[1] or
            ...
            doNotFit.extend([x for x in list(first_guess.keys()) if 'TEFF ' in x])
        if not any(['vrad' in o[1] or 'vpuls' in o[1]
                    or 'normalized spectrum' in o[1] for o in obs]):
            doNotFit.append('d_kpc')
            doNotFit.extend([x for x in list(first_guess.keys()) if 'VPULS ' in x])
        ...
        if 'P-FACTOR' in list(first_guess.keys()) and\
            'd_kpc' in list(first_guess.keys()) and\
    ...
    tmp = [x for x in list(first_guess.keys()) if isinstance(first_guess[x], str)]
    if len(tmp)>0:
        if doNotFit is None:
            doNotFit=tmp
        else:
            doNotFit.extend(tmp)
        try:
            fitOnly = [x for x in fitOnly if not isinstance(first_guess[x], str)]
        except:
            pass
    ...
    if N_monte_carlo>0:
        ...
        while len(mc_fits)<N_monte_carlo:
            ...
            mc_fits.append(tmp)
            if verbose:
                print(' '*4, 'chi2= %5.3f '%(tmp['chi2']), end=' ')
                print('dist= %6.4f pc '%(tmp['best']['d_kpc']), end=' ')
...
def fit2html(f, root='spips', directory='./', makePlots=True):
    ...
    for k in keys:
        if k in list(f['uncer'].keys()) and f['uncer'][k]>0:
...
def smartFit(obs, firstGuess, phaseDependent={'VPULS':('spline', 4),'TEFF':('fourier', 4)}, fitPeriod=False):
    ...
    obsp = [x for x in obs if x[1]=='vrad' or x[1]=='vpuls']
    ...
    fitOnly = [x for x in list(fg.keys()) if 'VPULS ' in x]
    if fitPeriod:
        fitOnly.extend([x for x in list(firstGuess.keys()) if 'PERIOD' in x])
...
def model(x, a, plot=False, starName=None, verbose=False, uncer=None, showOutliers=False,
    ...
    # -- excess exponent:
    if 'EXCESS SLOPE' in a and 'K EXCESS' in a:
        f_excess = lambda x: max(0, a['K EXCESS'] + (x-2.12)*a['EXCESS SLOPE'])*(x>=1.1)
        title_excess = 'IR$_{ex}$ = %5.3f + %5.3f(%s-2.12) mag'%(a['K EXCESS'],a['EXCESS SLOPE'], r'$\lambda$')
        k_excess =  f_excess(2.12)
    elif 'EXCESS EXP' in a and 'K EXCESS' in a:
        f_excess = lambda x: max(k_excess*(x/2.2)**a['EXCESS EXP'],0.)*(x>=1.1)
        title_excess = 'IR$_{ex}$ = %5.3f(%s/2.2)$^{%5.3f}$ mag'%(k_excess, r'$\lambda$', a['EXCESS EXP'])
        k_excess = f_excess(2.12)
    elif 'EXCESS SLOPE' in a and 'EXCESS WL0' in a:
        if 'EXCESS EXP' in a:
            f_excess = lambda x: a['EXCESS SLOPE']*max(x-a['EXCESS WL0'],0)**a['EXCESS EXP']
            title_excess = 'IR$_{ex}$ = %5.3f(%s - %5.3f)$^{%5.3f}$ mag'%(
                                        a['EXCESS SLOPE'], r'$\lambda$', a['EXCESS WL0'], a['EXCESS EXP'])
        else:
    ...
    if any(['VPULS VAL' in k for k in list(a.keys())]):
        # -- using splines:
        useFourierVpuls=False
        nvpuls = len([x for x in list(a.keys()) if 'VPULS VAL' in x])
        nx = nvpuls
        if nvpuls == 1:
            # -- checks if values are given with respect to first one:
            ndvpuls = len([x for x in list(a.keys()) if 'VPULS DVAL' in x])
            nx += ndvpuls
        if 'VPULS POW' in list(a.keys()):
            # -- nodes
    ...
    elif 'VPULS A0' in list(a.keys()):
        #-- assume Fourier coefficients:
        useFourierVpuls=True

        xvpuls = phi_intern

        Ns = [int(k.split('I')[1]) for k in [x for x in list(a.keys()) if 'VPULS PHI' in x]]
        Vpuls = np.ones(Nintern-1)*a['VPULS A0']

        for k in Ns:
            if 'VPULS A'+str(k) in a and \
               'VPULS PHI'+str(k) in a:
               # -- default behaviorr
                Vpuls += a['VPULS A'+str(k)]*np.cos(2*np.pi*k*xvpuls +
                                                    a['VPULS PHI'+str(k)])
            elif 'VPULS A1' in a and \
                 'VPULS PHI1' in a and \
                 'VPULS R'+str(k) in a and \
                 'VPULS PHI'+str(k) in a:
                # -- ratio of amplitudes
    ...
    if any(['VRAD VAL' in k for k in list(a.keys())]):
        # -- using splines:
        useFourierVpuls=False
        nvpuls = len([x for x in list(a.keys()) if 'VRAD VAL' in x])
        nx = nvpuls
        if nvpuls == 1:
            # -- checks if values are given with respect to first one:
            ndvpuls = len([x for x in list(a.keys()) if 'VRAD DVAL' in x])
            nx += ndvpuls
        else:
            ndvpuls = 0
        if 'VRAD POW' in list(a.keys()):
            # -- nodes
            #x0 = np.linspace(-0.5, 0.5, nvpuls)
        ...
        if verbose:
            print('VGAMMA:', round(Vgamma,3), 'km/s')
    elif 'VRAD A0' in list(a.keys()):
        #-- assume Fourier coefficients:
        useFourierVpuls=True

        xvpuls = phi_intern

        Ns = [int(k.split('I')[1]) for k in [x for x in list(a.keys()) if 'VRAD PHI' in x]]
        Vrad = np.ones(Nintern-1)*a['VRAD A0']

        for k in Ns:
            if 'VRAD A'+str(k) in a and \
               'VRAD PHI'+str(k) in a:
                # -- default behavior
                _phi = xvpuls + a['VRAD PHI'+str(k)]/(2*np.pi)/k
                Vrad += a['VRAD A'+str(k)]*np.cos(2*np.pi*k*_phi)
            elif 'VRAD A1' in a and \
                 'VRAD PHI1' in a and \
                 'VRAD R'+str(k) in a and \
                 'VRAD PHI'+str(k) in a:
                # -- ratio of amplitudes
    ...
    xt_pow = None
    if any([k.startswith('TEFF') for k in list(a.keys())]):
        if any(['TEFF VAL' in k for k in list(a.keys())]):
            # -- using splines:
            useSplineTeff=True
            # -- get the nodes
            nteff = len([x for x in list(a.keys()) if 'TEFF VAL' in x])
            nx = nteff
            if nteff == 1: # only val0 is defined
                ndteff = len([x for x in list(a.keys()) if 'TEFF DVAL' in x])
                nx += ndteff
            else:
                ndteff = 0
            nphi = len([x for x in list(a.keys()) if 'TEFF PHI' in x])
            if nphi < nteff or nphi < ndteff-1:
                # -- if comb is not defined, use the same one as in Vpuls
                if not 'TEFF PHI0' in list(a.keys()) and 'VPULS PHI0' in list(a.keys()):
                    a['TEFF PHI0'] = a['VPULS PHI0']
                if not 'TEFF POW' in list(a.keys()) and 'VPULS POW' in list(a.keys()):
                    a['TEFF POW'] = a['VPULS POW']
                if not 'TEFF PHI0' in list(a.keys()) and 'VRAD PHI0' in list(a.keys()):
                    a['TEFF PHI0'] = a['VRAD PHI0']
                if not 'TEFF POW' in list(a.keys()) and 'VRAD POW' in list(a.keys()):
            ...
            Ns = [int(k.split('I')[1]) for k in [x for x in list(a.keys()) if 'TEFF PHI' in x]]
            if 'TEFF A0' in a:
                Teff += a['TEFF A0']
            if any('TEFF R' in k for k in list(a.keys())):
            ...
            Ns = [int(k.split('S')[1]) for k in [x for x in list(a.keys()) if 'TEFF S' in x]]
            _L = 0.0
            for k in Ns:
        ...
        Lum = None
        if any(['LUM VAL' in k for k in list(a.keys())]):
            # -- using splines:
            useSplineLum=True
            # -- get the nodes
            nteff = len([x for x in list(a.keys()) if 'LUM PHI' in x])
        ...
        elif 'LUM A0' in list(a.keys()) or 'LUM A1' in list(a.keys()):
            #-- Fourier coefficients:
            useSplineLum=False
            Ns = [int(k.split('I')[1]) for k in [x for x in list(a.keys()) if 'LUM PHI' in x]]
    ...
    if 'LUM SLOPE' in list(a.keys()): # assumes at minimum LUM A0 was set
        _v = (Vpuls-Vgamma)
        origTeff = Teff.copy()
    ...
    elif 'Rspec PHI0' in list(a.keys()):
        nr = len([x for x in list(a.keys()) if 'Rspec PHI' in x])
    ...
    if not plot:
        ...
        for k, obs in enumerate(x):
            if obs[1].split(';')[0]=='negative E(B-V)':
                ...
                if phi[k] is None:
                    ...
                    if 'TEFF DATA OFFSET' in list(a.keys()):
                        res[-1] += a['TEFF DATA OFFSET']

            elif obs[1].split(';')[0]=='normalized spectrum':
                print('--- normalized spectrum...', end=' ')
        ...
        if any(['dMAG ' in k for k in list(a.keys())]):
        ...
        if not all(np.isfinite(res)):
            print("WARNING! nan or infinites detected in model's result", end=' ')
    ...
    if verbose:
        ...
        print('  - unred Mag for 1mas diameter, Teff=4500K, 5000, 5500K, 6000, 6500K, logg=1.5:')
    for l in list_all_filt:
        if __monochromaticAlambda:
            ...
            al = (Alambda_Exctinction(l, EB_V=1.0, Rv=Rv, Teff=4500),
                  #Alambda_Exctinction(l, EB_V=1.0, Rv=Rv, Teff=5000),
                  Alambda_Exctinction(l, EB_V=1.0, Rv=Rv, Teff=5500),
                  #Alambda_Exctinction(l, EB_V=1.0, Rv=Rv, Teff=6000),
        ...
        fits_model['MAG_1MAS 5000K_LOGG1.5 '+l] = round(photometrySED(1.0, 5000.0, l, logg=1.5, metal=a['METAL'])[0][0], 3)
        fits_model['MAG_1MAS 5500K_LOGG1.5 '+l] = round(photometrySED(1.0, 5500.0, l, logg=1.5, metal=a['METAL'])[0][0], 3)
        fits_model['MAG_1MAS 6000K_LOGG1.5 '+l] = round(photometrySED(1.0, 6000.0, l, logg=1.5, metal=a['METAL'])[0][0], 3)
        fits_model['MAG_1MAS 6500K_LOGG1.5 '+l] = round(photometrySED(1.0, 6500.0, l, logg=1.5, metal=a['METAL'])[0][0], 3)
        fits_model['MAG_1MAS 7000K_LOGG1.5 '+l] = round(photometrySED(1.0, 7000.0, l, logg=1.5, metal=a['METAL'])[0][0], 3)
    ...
    def labelPanel(uni, x0=-0.08):
        ...
        #          chr(uni), va='bottom', ha='left', fontweight='bold',
    ...
    if any(['VRAD 'in k for k in list(a.keys())]) and not useFourierVpuls:
        j = 0
    ...
    if any(['VPULS 'in k for k in list(a.keys())]) and not useFourierVpuls:
        j = 0
    ...
    if len(w[0])>0:
        ...
        if plot:
            plt.text(1.05, y_max,
                         r'V$_{puls}$ $\chi^2$=%4.2f'%chi2, va='top', ha='right',
                         size=10)
    ...
    if len(w[0])>0:
        ...
        if not useFourierVpuls and plot:
            if any(['VRAD 'in k for k in list(a.keys())]):
        ...
        if plot:
            plt.text(1.05, y_max+0.10*(y_max-y_min),
                    r'V$_{rad}$ $\chi^2$=%4.2f'%chi2, va='top', ha='right', size=10)
    if plot:
                        ...
                        if not np.isscalar(x[i][2]) and np.abs(x[i][2][0]-1.65)<0.3:
                            if plot:
                                ax2.errorbar(phi[i]+df, x[i][-2]/UD_LD, yerr=x[i][-1],
                                            ...
                                            alpha=0.7, markersize=4,
                                            label=r'UD_H->$\theta$Ross $\chi^2$=%3.1f'%chi2 if _label1 else '')
                            if _label1:
                                #print '  |%-30s:'%('UD_H'), chi2
    ...
    for i, filt in enumerate(list_filt):
        ...
        if verbose:
            print('%-20s = %5.3f '%(filt, np.mean(modl)-5*np.log10(a['d_kpc']/0.01)), end=' ')
        ...
        if plot:
            ...
            if splitDTeffects:
                plt.plot(xmo, np.array(modlT), color='y', linewidth=1, alpha=0.8,
                        linestyle='dotted', label=r'DeltaR=0')
                plt.plot(xmo, np.array(modlD), color='g', linewidth=1, alpha=0.8,
                        linestyle='dotted', label=r'DeltaTeff=0')
    ...
    for i, filt in enumerate(list_color):
        ...
        for x_ in xmo: # for each phase of the model
            modl.append(photometrySED([iDiam(x_), COMP_DIAM],
                                         ...
                                         filt.split('-')[0],
                                         logg=[ilogg_m(x_), COMP_LOGG],
                                    metal=a['METAL'] if 'METAL' in a else 0.0)-
    ...
    if any(['PERIOD' in k and k!='PERIOD' for k in list(a.keys())]):
    ...
    if plot and len(excess['wl'])>0:
        ...
        _X = [x for x in _X if x<=1.2*max(excess['wl']) and
                              x>=0.8*min(excess['wl'])]
        ax2.set_xticks(_X)
    ...
    if exportFits:
        ...
        for k in np.sort(list(fits_model.keys())):
        ...
        cols.append(pyfits.Column(name='MJD', format='D',
                array=np.array([None if o[0] is None else float(o[0]) for o in x])))
...
def importFits(fitsname, runSPIPS=False):
    ...
    for k in [x for x in f[0].header if x.startswith('PARAM ')]:
        a[k.split('PARAM ')[1]] = f[0].header[k]
    for k in [x for x in f[0].header if x.startswith('UNCER ')]:
        if f[0].header[k]>0:
...
def datasetPhaseCoverageQuality(obs):
    ...
    mags = [o for o in obs if o[1].startswith('mag') and o[-1]>0]
    Nvis, Nir = 0, 0
    if len(mags)>0:
...
def computePhaseOffset(obs, a, deltaMJD=1000., mjds=None):
    ...
    for i in range(len(mjds)-1):
        obs_ = [o for o in obs if o[0]>=mjds[i] and o[0]<mjds[i+1]]
        if len(obs_)>0:
            mjd_mean = np.mean([x[0] for x in obs_])
            print('%9.3f -> %9.3f (%8.3f) N=%3d'%(mjds[i], mjds[i+1], mjds[i+1]-mjds[i], len(obs_)), end=' ')
            chi2 = []
            for d in dP:
...
def phaseFunc(mjd, p, vgamma=0.0):
    ...
    if 'PERIOD1' in p: # in s/year
        __period += (__mjd_mjd0)*p['PERIOD1']/(24*3600*365.25)

    px = [x for x in list(p.keys()) if x.startswith('PERIOD') and
                x!='PERIOD' and x!='PERIOD1' and not x.startswith('PERIOD ')]
    if len(px)>0:
        for k in px:
    ...
    sinmod = set([k.split('PERIOD SIN PER')[1] for k in list(p.keys()) if k.startswith('PERIOD SIN PER')])
    for sm in sinmod:
...
def _showMagGrid(bands):
    """
    """
    print('# magnitude for a 1mas angular diameter star,', end=' ')
...
def _makeAll_photometryGrid():
    ...
    for f in np.sort(list(photfilt2._data.keys())): # for each filtername
        print(f)
...
def photometrySED(diam, teff, filtname, metal=0.0, plot=False, Nwl=100,
    ...
    if useGrid:
        if filtname in list(__MAGgrid.keys()):
...
def allFilter():
    ...
    for i,f in enumerate(filters):
        print(f.split('_')[0]+'$_\mathrm{'+f.split('_')[1]+'}$', '&', end=' ')
        print(round(photfilt2.effWavelength_um(f)*1000, 1), '&', end=' ')
        ...
        print(photfilt2._data[f]['ID'], '&', end=' ')
        print(photfilt2._data[f]['Description'], end=' ')
        print('&', '(%d)'%ref[i], r'\\')
    ...
    for i,f in enumerate(filters):
        ...
        print('%5.3f, %5.3f, %5.3f'%(photometrySED(1.0, 4500.0, f, logg=1.5),
                                     photometrySED(1.0, 5500.0, f, logg=1.5),
                                     photometrySED(1.0, 6500.0, f, logg=1.5),), '&', end=' ')
        print('%5.3f, %5.3f, %5.3f'%(Alambda_Exctinction(f, EB_V=1, Rv=3.1, Teff=4500),
                                    Alambda_Exctinction(f, EB_V=1, Rv=3.1, Teff=5500),
                                     Alambda_Exctinction(f, EB_V=1, Rv=3.1, Teff=6500)), end=' ')
        print('\\')
        ...
        for n in [1,2]:
            ...
            for k,x in enumerate(c[::-1]):
                if k==0:
                    ...
                    print('%s%4.2f t'%('+' if x>=0 else '', x), end=' ')
                else:
                    print('%s%4.2f t^%d'%('+' if x>=0 else '', x,k), end=' ')
            # -- residuals
            res =  m-np.polyval(c, X)
            print(r'$ & %4.3f '%(res.ptp()), end=' ')
            if n!=2:
                print('&', end=' ')
        print(r'\\')
